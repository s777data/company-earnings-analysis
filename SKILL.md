...
- Quote and broker fundamentals: `robinhood-trading` MCP only. During regular market hours, require a quote no older than 15 minutes. For a production run after the regular session closes, use the newest completed daily regular-session close; if the daily series lags a newer broker-stamped 15:55–16:05 ET regular-session last trade, use that newer closing trade and preserve its exact venue timestamp. This completed-close path remains production data and must not be labeled test-only or stale. Never substitute extended-hours pricing. `--test-allow-stale-quote` remains restricted to explicit tests using older non-close data.
...
Supported transcript hosts include StockAnalysis, Seeking Alpha, Motley Fool, MarketBeat, StreetInsider, and Investing.com. The accepted transcript URL is retained in every output. For StockAnalysis transcript-detail URLs, first request the ticker transcript index in the same HTTP session, then request the detail URL with the index as `Referer`; direct detail downloads may return `Invalid download request`. Validate a structured-provider candidate immediately before issuing slower search-engine fallback queries.
...
- KPI — twelve readable source-derived operating scorecards ranked by importance
- Key Ratios
- Valuation
