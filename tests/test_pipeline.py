...
class InteractiveDashboardTests(unittest.TestCase):
    ...
    def test_reference_scorecard_structure_is_shared_and_data_driven(self):
        ...
        self.assertIn("1cqw", css)
        self.assertIn("1.15cqh", css)
        ...
        self.assertIn(".kpi-yoy--positive", css)
        self.assertIn(".kpi-current-value", css)
        ...
        self.assertIn('const yoy = kpiYoy(metric)', script)
        self.assertIn('yoyNode.className = `kpi-yoy ${yoy.className}`', script)
        self.assertIn('current.className = "kpi-current-value"', script)
